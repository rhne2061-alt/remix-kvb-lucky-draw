...
export default function GridLottery({
  ...
  const runSpinPlan = (result: NonNullable<typeof drawnResult>) => {
    ...
    if (targetIndex < 0) {
      // === FIX: if the won prize is not part of the visible 8-cell grid
      //             (e.g. downgraded or off-grid prize), still animate to a
      //             sensible cell so the user sees a result instead of a
      //             silently reset button ===
      targetIndex = 0;
    }
