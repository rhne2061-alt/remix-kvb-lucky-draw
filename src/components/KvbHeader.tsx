...
export default function KvbHeader({ lang = 'id', customLogo, onToggleLang }: KvbHeaderProps) {
      ...
      <header id="kvb-header" className="bg-white/5 backdrop-blur-sm border-b border-white/10 px-6 py-3 md:py-4 sticky top-0 z-50 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] select-none transition-all">
        <div className="max-w-7xl mx-auto flex items-center justify-center md:justify-start">
          <img 
            ...
            decoding="async"
            onError={() => {
              console.error('[KvbHeader] Failed to load custom logo');
              setLogoError(true);
            }}
          />
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center">
                <svg className="h-9 w-auto select-none shrink-0" viewBox="0 0 140 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M 119,11 C 121,9 121,5 119,3 C 117,1 117,7 119,11" fill="#fbbf24" />
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 items-center justify-end text-[11px] font-mono text-slate-600">
          ...
          <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-blue-700 font-bold tracking-tight">
            <span className="flex items-center gap-1">
               ...
               {lang === 'zh' ? '2026 联合专属' : 'UNITED 2026 EXCLUSIVE'}
            </span>
          </div>

          {/* === FIX #1: language switch button so users can actually flip between zh / id === */}
          {onToggleLang && (
            <button
              type="button"
              onClick={onToggleLang}
              title="Toggle language / 切换语言"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-slate-700 font-bold text-[10px] tracking-wider hover:bg-slate-200 transition-colors"
            >
              <Languages className="h-3 w-3" />
              <span>{lang === 'zh' ? '中文' : 'ID'}</span>
              <span className="opacity-40">|</span>
              <span className="opacity-60">{lang === 'zh' ? 'ID' : '中文'}</span>
            </button>
          )}
        </div>
      </div>
