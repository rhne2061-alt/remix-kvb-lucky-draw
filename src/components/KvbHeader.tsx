...
export default function KvbHeader({ lang = 'id', customLogo }: KvbHeaderProps) {
  ...
  const prevSrcRef = useRef(customLogo);

  useEffect(() => {
    if (customLogo !== prevSrcRef.current) {
      prevSrcRef.current = customLogo;
      setLogoError(false);
      setLogoLoaded(false);
    }
  }, [customLogo]);
        ...
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
          <div className="flex items-center bg-transparent p-1">
            <div className="flex items-center gap-3">
              {/* 自定义 Logo 或 默认 SVG Logo - 保持布局统一，避免DOM切换闪烁 */}
              {customLogo && !logoError ? (
                <div className="relative flex items-center" style={{ minWidth: 80, minHeight: 50 }}>
                  {!logoLoaded && (
                  ...
                  <img
                    ...
                    className="max-h-[70px] w-auto object-contain transition-opacity duration-500"
