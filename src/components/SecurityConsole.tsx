...
import { uploadPrizeImage, deletePrizeImage, uploadCustomImage, deleteCustomImage } from "../firebase";
...
interface SecurityConsoleProps {
  ...
  /**
   * === FIX: split the prize image callback into "URL" and "Base64"
   *             variants. The Storage URL is the new primary path;
   *             base64 is still supported for the legacy/offline
   *             fallback so the operator can switch projects without
   *             losing their data. ===
   */
  onUpdatePrizeImage?: (
    prizeId: string,
  ...
  onClearPrizeImage?: (prizeId: string) => void;
...
export default function SecurityConsole({
  ...
  // === FIX: default tab is now "gambar" (image upload) so the operator
  //             can immediately upload real prize photos without first
  //             hunting through the other 6 tabs. The previous default
  //             ("invitation") was misleading because there was no
  //             visible affordance for the image upload section. ===
  ...
  const handleImageUpload = async (params: {
    ...
    /**
     * === FIX: callback now receives a URL (preferred) and/or a
     *             base64 string. The Firebase Storage path always
     *             passes a URL; the legacy base64 path is preserved
     *             for offline builds / no-Storage projects. ===
     */
    onSuccess: (payload: { url?: string; base64?: string }) => void;
    /**
     * Optional Storage path. When provided, the upload will be sent
     * to Firebase Storage and the download URL will be returned via
     * `onSuccess`. When omitted, the upload falls back to base64 only.
     */
    storagePath?: string;
  }) => {
    ...
    try {
      const blob = await compressImageFileToBlob(
        params.file,
        params.compression,
      );
      console.info(
        `[SecurityConsole] compressed blob ready, ${blob.size} bytes, mime=${blob.type || params.compression.mimeType}`,
      );

      if (params.storagePath) {
        // Use the appropriate upload function based on the storage path
        const isCustomImage = params.storagePath.startsWith("custom-");
        let uploadResult: { url: string; storagePath: string };
        
        if (isCustomImage) {
          // Upload custom bg/logo to Firebase Storage
          uploadResult = await uploadCustomImage(
            params.storagePath === "custom-bg" ? "bg" : "logo",
            blob,
            blob.type || params.compression.mimeType,
          );
        } else {
          // Upload prize image to Firebase Storage
          uploadResult = await uploadPrizeImage(
            params.storagePath,
            blob,
            blob.type || params.compression.mimeType,
          );
        ...
        setUploadFeedback({
          ...
          message:
            lang === "zh"
              ? `✅ 上传成功(${Math.max(1, Math.round(blob.size / 1024))} KB,已发布到 CDN)`
              : `✅ Berhasil diunggah (${Math.max(1, Math.round(blob.size / 1024))} KB, dipublikasikan ke CDN)`,
        });
        window.setTimeout(() => setUploadFeedback(null), 4000);
        params.onSuccess({ url: uploadResult.url });
        return;
      }

      // Legacy base64 path — used when no `storagePath` is given.
      ...
      console.info(
        `[SecurityConsole] prize image ready, dataUrl length=${dataUrl.length} (~${approxBytes} bytes), mime=${dataUrl.slice(5, dataUrl.indexOf(";"))}`,
      );
      setUploadFeedback({
        type: "success",
        message:
          lang === "zh"
            ? `✅ 图片已上传成功(${Math.max(1, Math.round(approxBytes / 1024))} KB)`
            : `✅ Gambar berhasil diunggah (${Math.max(1, Math.round(approxBytes / 1024))} KB).`,
      });
      window.setTimeout(() => setUploadFeedback(null), 4000);
      params.onSuccess({ base64: dataUrl });
    } catch (err: any) {
      console.error("[SecurityConsole] image upload failed:", err);
      setUploadFeedback({
        type: "error",
        message:
          lang === "zh"
      ...
      <div className="bg-transparent px-6 py-4 border-b border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs transition-colors border border-slate-700 cursor-pointer active:scale-95 font-sans"
          >
        ...
        {activeTab === "invitation" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="pt-4 border-t border-slate-800/80">
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-amber-500/80 to-amber-600/80 hover:from-amber-600 hover:to-amber-755 text-slate-950 text-xs py-2.5 px-4 rounded-xl font-black cursor-pointer transition-all active:scale-95 flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                      ...
                      className="w-full bg-slate-800 hover:bg-slate-700 text-amber-400 border border-amber-500/30 text-xs py-2 px-4 rounded-xl font-bold cursor-pointer transition-all active:scale-95 flex items-center justify-center gap-2"
                    >
              ...
              <div className="lg:col-span-7 bg-[#090f20]/40 p-5 rounded-xl border border-slate-800 flex flex-col h-[340px]">
        ...
        {activeTab === "sheets" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-[#090f20]/60 p-5 rounded-xl border border-slate-800 space-y-4">
                ...
                <div className="pt-3 border-t border-slate-800 flex flex-col sm:flex-row gap-2">
                  <button
                    ...
                    className="w-full text-center bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs py-2 px-4 rounded-xl border border-slate-700 font-bold cursor-pointer transition-all disabled:opacity-50 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-center bg-gradient-to-r from-emerald-500/80 to-teal-600/80 hover:from-emerald-600 hover:to-teal-700 text-white text-xs py-2 px-4 rounded-xl font-bold cursor-pointer transition-all disabled:opacity-50 active:scale-95"
                  >
                    {syncLoading
                      ? "..."
              ...
              <div className="bg-[#090f20]/40 p-5 rounded-xl border border-slate-800 flex flex-col justify-between">
                ...
                <div className="mt-4 border border-slate-800 rounded-xl overflow-hidden bg-black/50 text-left flex flex-col">
                  ...
                  <pre className="p-3.5 font-mono text-[9px] text-slate-400 overflow-x-auto max-h-[140px] leading-relaxed select-all">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      void handleImageUpload({
                        ...
                        storagePath: "custom-bg",
                        onSuccess: (payload) =>
                          onUpdateCustomBg?.(payload.url || payload.base64 || ""),
                      });
                    }}
                  />
                  {onUndoCustomBg && (
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      void handleImageUpload({
                        ...
                        storagePath: "custom-logo",
                        onSuccess: (payload) =>
                          onUpdateCustomLogo?.(payload.url || payload.base64 || ""),
                      });
                    }}
                  />
                  {onUndoCustomLogo && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {prizes.map((p) => {
                const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
                  ...
                  void handleImageUpload({
                    file,
                    // === FIX: bumped the upload size limit from 2 MB to 8 MB
                    //             so the operator can upload a real
                    //             high-resolution prize photo. The previous
                    //             2 MB cap was a hidden rejection wall for
                    //             anyone using a phone camera JPEG. ===
                    maxFileSizeBytes: 8 * 1024 * 1024,
                    compression: {
                      // === FIX: 200 px was destroying every uploaded
                      //             photo (the cell is ~80 px and the
                      //             modal is 280 px, so 200 px looked
                      //             blurry in both). 1024 px is enough
                      //             for the 280 px win modal at 1x and
                      //             2x DPI, and keeps the dataUrl under
                      //             ~200 KB after WebP compression. ===
                      maxSize: 1024,
                      mimeType: "image/webp",
                      quality: 0.9,
                    },
                    storagePath: p.id,
                    onSuccess: (payload) => onUpdatePrizeImage?.(p.id, payload),
                  });
                };
                ...
                return (
                  ...
                  >
                    ...
                    {(p.customImageBase64 || p.customImageUrl) && (
                      <button
                        onClick={() => onClearPrizeImage?.(p.id)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-1 bg-slate-900/40 p-4 rounded-xl border border-slate-800 flex flex-col justify-between">
                ...
                <button
                  ...
                  className="w-full mt-4 bg-gradient-to-r from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 text-white font-bold text-xs py-2.5 px-4 rounded-xl shadow cursor-pointer active:scale-95 disabled:opacity-50 flex items-center justify-center gap-1.5"
                >
              ...
              {/* Console log outputs */}
              <div className="md:col-span-2 bg-black border border-slate-800 rounded-xl p-4 font-mono text-[10px] h-[160px] overflow-y-auto text-slate-300 flex flex-col">
                <div className="flex items-center justify-between border-b border-slate-800 pb-1 mb-2 text-slate-500 shrink-0">
                ...
                <div className="flex-1 overflow-y-auto space-y-1.5 scrollbar-thin">
                  {attackOutput.length === 0 ? (
                    <span className="text-slate-500 italic leading-loose block">
                      {t.terminalEmpty}
                    </span>
                  ) : (
