...
export const generateMockDrawHistory = () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    // === FIX: the previous mock never surfaced the high-tier prizes
    //             (`gold10g / ninja250 / macbook / iphone16`) in the
    //             audit / live feed demo, even though they're real. Add
    //             a small chance for each so the operator console can
    //             actually show what a "Coba Lagi Besok" downgrade or a
    //             Failsafe hit looks like. ===
    let winner =
      INITIAL_PRIZES.find((p) => p.id === "gold_guide") || INITIAL_PRIZES[5];
    if (roll < 0.04) {
      winner =
        INITIAL_PRIZES.find((p) => p.id === "iphone16") || INITIAL_PRIZES[6];
    } else if (roll < 0.07) {
      winner =
        INITIAL_PRIZES.find((p) => p.id === "macbook") || INITIAL_PRIZES[4];
    } else if (roll < 0.10) {
      winner =
        INITIAL_PRIZES.find((p) => p.id === "gold10g") || INITIAL_PRIZES[0];
    } else if (roll < 0.15) {
      winner =
        INITIAL_PRIZES.find((p) => p.id === "trade_signal") ||
        INITIAL_PRIZES[1];
    } else if (roll < 0.35) {
      winner =
        INITIAL_PRIZES.find((p) => p.id === "whitepaper") || INITIAL_PRIZES[3];
    } else if (roll < 0.45) {
      winner =
        INITIAL_PRIZES.find((p) => p.id === "vip_slot") || INITIAL_PRIZES[7];
    }

    const timeOffset = new Date(
      now.getTime() - (i * 12 + Math.floor(Math.random() * 45)) * 60 * 1000,
    );
