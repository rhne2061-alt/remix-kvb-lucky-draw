  ...
  if (config.syncMethod === 'webapp') {
    ...
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          error: `Apps Script 返回 HTTP ${response.status}`,
        };
      }
      return { success: true };
    } catch (error: any) {
      // === FIX: previously this fallback claimed `success: true` regardless
      //             of the network outcome, so any CORS/transport error
      //             silently looked like a successful upload. Now we keep
      //             the no-cors last-ditch attempt but only return success
      //             if the fetch itself resolved (not rejected). Note that
      //             no-cors returns an opaque response, so we cannot read
      //             the status — callers should still verify the row. ===
      try {
        ...
        return {
          success: false,
          error: `no-cors 写回状态 ${resp.status}`,
        };
      } catch (innerErr: any) {
